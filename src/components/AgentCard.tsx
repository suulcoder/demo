...
export function AgentCard({ agent, onSelect }: AgentCardProps) {
  ...
  return (
    <article className="group flex flex-col rounded-2xl border border-border bg-surface-elevated p-6 transition-colors hover:border-accent/40">
      ...
      <Button
        ...
        aria-label={`Probar agente para ${agent.title}`}
      >
        {isPlaceholder ? 'Próximamente' : 'Probar agente'}
      </Button>
    </article>
  )
}
